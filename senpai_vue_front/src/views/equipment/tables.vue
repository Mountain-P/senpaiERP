<template>
  <v-container>
    <v-data-table :headers="headers" :items="contents" class="elevation-1">
      <template v-slot:top>
        <v-toolbar flat>
          <v-toolbar-title>器材列表</v-toolbar-title>
                ...
                <v-container>
                  <v-row>
                    <v-col cols="12" sm="12" md="12">
                      <v-text-field v-model="editedItem.number" label="器材編號"></v-text-field>
                    </v-col>
                    <v-col cols="12" sm="12" md="12">
                      <v-text-field v-model="editedItem.brand" label="器材廠牌"></v-text-field>
                    </v-col>
                    <v-col cols="6" sm="12" md="12">
                      <v-text-field v-model="editedItem.name" label="器材名稱"></v-text-field>
                    </v-col>
                    <v-col cols="6" sm="12" md="6">
                      <v-text-field v-model="editedItem.purchaseDate" label="購買日期"></v-text-field>
                    </v-col>
                    <v-col cols="6" sm="12" md="6">
                      <v-text-field v-model="editedItem.price" label="購買價格"></v-text-field>
                    </v-col>
                    <v-col cols="12" sm="12" md="12">
                      <v-text-field v-model="editedItem.accessory" label="器材配件"></v-text-field>
                    </v-col>
                    <v-col cols="12" sm=12 md="12">
                      <v-file-input v-model="editItem.picture" accept="image/*" label="器材照片"></v-file-input>
                    </v-col>
                  </v-row>
                </v-container>
              </v-card-text>
...
export default {
  ...
  data() {
    return {
      ...
      datePicker: false,
      headers: [
        { text: "器材名稱", value: "name" },
        { text: "器材品牌", value: "brand" },
        { text: "是否在庫", value: "inStock" },
      ],
      contents: [],
      editedIndex: -1,
      editedItem: {
  ...
  computed: {
    //結合
    formTitle() {
      return this.editedIndex === -1 ? "新增器材" : "編輯器材";
    },
  },
  watch: {
    //監看 新增/編輯 對話視窗是否有被關閉
